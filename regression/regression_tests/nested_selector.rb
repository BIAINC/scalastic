module RegressionTests
  module NestedSelector
    ...
    def run
      ...
      RegressionTests.prepare_index "integer", 'nested_selector'     # Must be called only once for the index

      # Indexing should work
      partition = partitions.create index: 'nested_selector', id: 1
      partition.index type: 'test', id: 1, body: {title: 'Test', content: 'This is a test'}

      sleep 1.5

      # Searching should work, too
      results = partition.search(type: 'test', body: {query: {match_all: {}}})
      ...
      partition.delete type: 'test', id: 1
      sleep 1.5
